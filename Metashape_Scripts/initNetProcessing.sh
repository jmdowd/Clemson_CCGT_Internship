...
# This script will submit all the necessary jobs in order to set up a Metashape parallel processing network, submit data for processing, and process
# the submitted data 
...
metashapeVar=`sed -n '5 p' ~/arguments.txt`
#Gets the 6th line of arguments.txt
dir=`sed -n '6 p' ~/arguments.txt`

#Submit a job that will start a Metashape server node using the script written in initServerNode.sh
...
#Sleep allows time for initServerNode.sh to write the IP Address to a file and allow the server to start running prior to moving on
sleep 30s

#Submit a job that currently creates 8 parallel Metashape processing nodes 
