...
# a python script that will use the parallel processing nodes to build the depth maps for the users project
...
#Starts Metashape offscreen and runs the Python Script BuildDepthMaps.py and passes arguments.txt as an argument to the script
sh $metashape/metashape.sh -platform offscreen -r $ad/BuildDepthMaps.py $cwd/arguments.txt
