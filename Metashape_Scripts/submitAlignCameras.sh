...
# a python script that will use the parallel processing nodes to align all the cameras in the users project
...
#Starts Metashape offscreen and runs the Python Script AlignCameras.py and passes arguments.txt as an argument to the script
sh $metashape/metashape.sh -platform offscreen -r $ad/AlignCameras.py $cwd/arguments.txt
