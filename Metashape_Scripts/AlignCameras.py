import Metashape, glob, sys, time

# AlignCameras.py
# Justin Dowd, Clemson Center for Geospatial Technologies, www.clemsongis.org
# March 2018
# Email: jmdowd@g.clemson.edu
# Scripts submits a task that aligns all photos in our previously created project
...
# Create a Metashape Network Task object called 'task2'
task2 = Metashape.NetworkTask()
...
# Connect to the IP Address of the Metashape server node we read in from address.txt
client.connect(addr_str)
# Create a batch to submit to the server node
batch_id2 = client.createBatch(filePath, [task2])
...
stat = client.batchStatus(batch_id2)
...
while(stat['status'] == "inprogress" or stat['status'] == "working"):
        stat = client.batchStatus(batch_id2)
		# Makes the loop sleep for 3 minutes before checking the status again
